...
class DatasetPage(DataSetMixin, DatasetSearchMixin, TypesetBodyMixin, HeroMixin, Page):
    ...
    def get_usages(self):
        dashboards = (
            ...
            .filter(
                models.Q(
                    narrativedashboardpage__dashboard_datasets__dataset__slug=self.slug
                )
            )
            .specific()
        )
...
class DatasetListing(HeroMixin, Page):
    ...
    def fetch_filtered_data(self, context):
        ...
        if topic:
            datasets = (
                ...
                .specific()
                .filter(dataset_topics__topic__slug=topic)
            )
        else:
            ...
            if "all--" in district:
                try:
                    ...
                    datasets = datasets.filter(
                        page_districts__district__region__name=region
                    )
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        context["topics"] = set(
            [
                page_orderable.topic
                for page_orderable in DatasetPageTopic.objects.all()
