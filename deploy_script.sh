...
APP_DIR=$SCRIPT_DIR
DOMAIN="live/masindi.datahub.go.ug"
...
if [ ! -f $FULL_CHAIN_DESTINATION ]; then
    ...
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout $PRIV_KEY_DESTINATION -out $FULL_CHAIN_DESTINATION -subj "/C=UG/ST=Kampala/L=Kampala/O=Global Security/OU=Devs/CN=masindi.datahub.go.ug"
fi
